...
MURKA_REPLIES = {
    ...
    "мурка какая твоя любимая песня": ["Aerosmith - What It Takes! ууу🐺💃", "audio"],
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ответы Мурки (проверка по длине ключа, длинные фразы первыми)
    for key, answers in sorted(MURKA_REPLIES.items(), key=lambda x: -len(x[0])):
        if key in text:
            if "audio" in answers:
                try:
                    with open("song.ogg", "rb") as voice:
                        await msg.reply_voice(voice)
                except Exception:
                    await msg.reply_text("Ой, голос потеряла 😿")
            else:
